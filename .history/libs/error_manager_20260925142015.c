...
static const char* ERROR_TYPE_STRING[18] =
{
    ...
    "_ADRESS_OUT_OF_RANGE_", // A adress you entered is not within the correct range
    "_NEGATIVE_LINE_ADRESS_" // Negative line adress entered
};

void error(const unsigned int index, const size_t line, const size_t char_, const int data_1, const int data_2, const int data_3, const char* string)
{
    printf("\n");
    switch (index)
    {
    ...
    case _NO_PATH_: // No path has been specified
        printf("KASM.exe [source path] [destination path]\n");
        break;
    ...
    case _VALUE_OUT_OF_RANGE_: // The value entered is not within the correct range
        printf("ERROR %s : The entered value is out of range for %s type [Min : %i , Max : %i] ", ERROR_TYPE_STRING[index], TOKEN_TYPE_STRING[data_3], data_1, data_2);
        printf("ln:%u,ch:%u\n", line, char_);
        break;
    
    case _ADRESS_OUT_OF_RANGE_: // The adress entered is not within the correct range
        printf("ERROR %s : The entered adress is out of range for %s type [Min : %i , Max : %i] ", ERROR_TYPE_STRING[index], TOKEN_TYPE_STRING[data_3], data_1, data_2);
        printf("ln:%u,ch:%u\n", line, char_);
        break;
    
    case _NEGATIVE_INSTRUCTION_INDEX_: // Negative line adress entered
        printf("ERROR %s : The entered line adress negative ", ERROR_TYPE_STRING[index]);
        printf("ln:%u,ch:%u\n", line, char_);
        break;
    
    case _INSTRUCTION_ADRESS_TOO_FAR_: // The instruction you tried to reach does not exist
        printf("ERROR %s : The instruction you tried to reach does not exist, max instruction adress : %i", ERROR_TYPE_STRING[index]);
        printf("ln:%u,ch:%u\n", line, char_);
        break;
