...
void synthaxe_debug(struct token* token_list)
{
    ...
    while (current_token->next)
    {
        ...
        else if(instruction_type == -1) // If we are not in a instrutction
        {
        ...
        else // If it's the end of the instruction
        {
            ...
            {
                error(_TOO_M_ARG_, instruction_line, instruction_char, argument_counter, "", number_of_argument_per_instruction[instruction_type]);
            }
        }
        if(current_token->next) // If the next token's pointer is valid
        {   
            current_token = current_token->next;
        }
        else if(instruction_type != -1) // If the token list reach his end but the last instruction is not closed
        {
