...
void error(unsigned int index, size_t line, size_t char_, int data_1, char* string, int data_2)
{
    switch (index)
    {
    ...
    case _FIRST_T_NOT_K_: // Unexpected asterisks
        printf("ERROR %u : Instruction must begin by a keyword, yours begin by a %s \n", index, string);
        printf("ln:%u,ch:%u\n", line, char_);
        break;
    
    case _TOO_M_ARG_: // Too many argument in the instruction
        printf("ERROR %u : Too many argument in the instruction [Exp : %i, Rec : %i]", index, data_2, data_1);
        printf("ln:%u,ch:%u\n", line, char_);
        break;
