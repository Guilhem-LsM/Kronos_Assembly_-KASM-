...
static const char* ERROR_TYPE_STRING[15] =
{
...
void error(const unsigned int index, const size_t line, const size_t char_, const int data_1, const char* string, const int data_2)
{
    ...
    case _VALUE_OUT_OF_RANGE_: // A value you entered for the type xxx is not within the correct range
        printf("ERROR %s : A value you entered is not within the correct range [Min : %i , Max : %i] ", ERROR_TYPE_STRING[index], TOKEN_TYPE_STRING[data_1]);
        printf("ln:%u,ch:%u\n", line, char_);
        break;
