...
void synthaxe_debug(struct token* token_list)
{
    ...
    while (current_token)
    {
        ...
        {
            if((current_token->type == _RAM_ADRESS_ || current_token->type == _REGISTER_ADRESS_) && INSTRUCTIONS_ARCHITECTURE[instruction_type][argument_counter] != _RAM_REGISTER_ADRESS_)
            {
                printf("%i\n",token_type_string[INSTRUCTIONS_ARCHITECTURE[instruction_type][argument_counter]]);
            ...
            else if(current_token->type != INSTRUCTIONS_ARCHITECTURE[instruction_type][argument_counter] && (current_token->type == _RAM_ADRESS_ || current_token->type == _REGISTER_ADRESS_))
            {
                error(_WRONG_ARG_TYPE_, current_token->line, current_token->char_, INSTRUCTIONS_ARCHITECTURE[instruction_type][argument_counter], "", current_token->type);
            }
            argument_counter++;
