...
const char VALID_KEYWORDS[21][7] =
{
    "noop",
    "load",
    "mov",
...
bool is_string_in_array(char* string, const char* array, size_t array_size)
{
    for(int i = 0; i < array_size; i++)
    {
    ...
    {   
        ...
        if(is_char_in_array(*char_pointer, SEPARATION_CHARS, SEPARATION_CHAR_NUMBER)) // If the char is a separator
        {
            if(in_lexeme)
            {
                ...
                {
                    if(!is_string_in_array(lexeme, VALID_KEYWORDS, VALID_KEYWORD_NUMBER)) // If keyword is invalid
                    {
