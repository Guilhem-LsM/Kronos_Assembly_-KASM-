...
// Consts
static const int NUMBER_OF_ARGUMENT_PER_INSTRUCTION[] = 
{
    0, 3, 3, 2, 2, 3, 3, 3, 2, 3, 3, 3, 3, 3, 3, 2, 2, 1, 2, 2, 2, 1, 0, 1
};

static const int INSTRUCTIONS_ARCHITECTURE[24][3] = 
{
    {_NULL_},
    ...
    {_REGISTER_ADRESS_, _INSTRUCTION_ADRESS_},
    {_INSTRUCTION_ADRESS_},
...
// Functions
    ...
    //Public
void debug_synthax(struct token* token_list)
{
    ...
    while (current_token)
    {
        ...
        {
            error(_FIRST_T_NOT_K_, current_token->line, current_token->char_, 0, 0, 0, TOKEN_TYPE_STRING[current_token->type]);
        }
        ...
        {   
                ...
                && current_token->type != _REGISTER_ADRESS_))
            {
                error(
                    ...
                    current_token->line, 
                    current_token->char_, 
                    INSTRUCTIONS_ARCHITECTURE[instruction_type][argument_counter], 
                    current_token->type,
                    0,
                    ""
                );
        ...
        if(current_token->type != _RAM_ADRESS_ && current_token->type != _REGISTER_ADRESS_)
        {
            ...
            {
                error(_DEREFERENCE_W_TYPE_, current_token->line, current_token->char_, current_token->type, 0, 0, "");
            }
        }

        if(instruction_type != -1 && !current_token->next) // If the token list reach his end but the last instruction is not closed
        {
            error(_EXPECTED_SEMICOLON_, instruction_line, instruction_char, 0, 0, 0, "");
        }
